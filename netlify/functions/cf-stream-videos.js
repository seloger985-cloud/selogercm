...
exports.handler = async function (event) {
  ...
  try {
    ...
    /* Filtrer par meta.section */
    const filtered = (data.result || [])
      .filter(v => {
        if (!(v.status?.state === 'ready' || v.readyToStream === true)) return false;
        if (tag === 'all') {
          /* Exclure les vidéos orphelines (pas de lien Supabase) */
          return !!(v.meta?.listing_id || v.meta?.['downloaded-from']);
        }
        return v.meta?.section === tag;
      })
      .slice(0, limit);

    /* Enrichir depuis Supabase par listing_id */
    let listingsMap = {};
